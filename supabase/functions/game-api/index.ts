...
function currentElo(session: Session) {
  if (session.client_version === LEGACY_CLIENT_VERSION) {
    return LEGACY_ELO_BASE[session.level] + session.decision_elo + session.exercise_elo;
  }
  return scoreElo(session.level, session.decision_elo, session.exercise_elo);
}
...
function publicSession(session: Session) {
  return {
    ...
    cheatingAccepted: Boolean(session.cheating_accepted),
...
function enrichProgress(session: Session, changes: Session) {
  ...
  changes.max_elo = Math.min(maxElo(MAX_LEVEL), Math.max(session.max_elo, elo));
